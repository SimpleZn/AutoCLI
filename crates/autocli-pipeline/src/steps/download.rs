...
async fn execute_article_download(
    ...
    let safe_title: String = title
        ...
        .take(80)
        .collect();
    let article_dir = std::path::PathBuf::from(output_dir)
        .join(&safe_title)
        .to_string_lossy()
        .to_string();
    ...
    if !image_urls.is_empty() {
        let images_dir = std::path::PathBuf::from(&article_dir)
            .join("images")
            .to_string_lossy()
            .to_string();
        ...
        for raw_url in &image_urls {
            ...
            let img_path = std::path::PathBuf::from(&images_dir)
                .join(&img_filename)
                .to_string_lossy()
                .to_string();
    ...
    // Write markdown file
    let file_path = std::path::PathBuf::from(&article_dir)
        .join(&filename)
        .to_string_lossy()
        .to_string();
...
async fn execute_media_batch_download(
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        if media_type == "image" {
            ...
            let filepath = std::path::PathBuf::from(&output_dir)
                .join(&filename)
                .to_string_lossy()
                .to_string();
...
async fn execute_ytdlp(
    ...
    let cookie_file = if !cookies_str.is_empty() {
        let cookie_path = std::path::PathBuf::from(&output_dir)
            .join(".ytdlp_cookies.txt")
            .to_string_lossy()
            .to_string();
        let domain = url
            .strip_prefix("https://")
